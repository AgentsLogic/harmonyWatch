...
export function VideoUploadDropzone({
  ...
  // Poll asset status to check when video is ready
  const pollAssetStatus = useCallback(async (uploadId: string, attempt = 0) => {
    const maxAttempts = 120; // 20 minutes max (10s intervals) - for long videos
    const interval = 10000; // 10 seconds

    if (attempt >= maxAttempts) {
      console.error('[Mux Upload] Asset polling timeout after 20 minutes');
      onUploadError('Video processing timeout after 20 minutes. Please try again or contact support if the issue persists.');
    ...
    try {
      ...
      const response = await fetch(`/api/upload/video-mux?uploadId=${uploadId}`);
      ...
      if (response.ok) {
        ...
        if (data.status === 'ready' && data.playbackId) {
          ...
          onUploadComplete({
            ...
            thumbnailUrl: undefined, // Mux will generate thumbnails automatically
          ...
          onUploadError('Video processing failed. Please try again.');
          setIsProcessing(false);
          return;
        }
      }
  ...
  const handleUploadSuccess = useCallback((event: any) => {
    ...
    if (uploadId) {
      console.log('[Mux Upload] Upload ID found:', uploadId);
      console.log('[Mux Upload] Filename state:', filename || eventFilename);
      setUploadId(uploadId);
      setIsProcessing(true);
      
      // Immediately notify parent that upload is complete (but still processing)
      // This allows saving content while video processes in background
      onUploadComplete({
        uploadId: uploadId,
      ...
      console.error('[Mux Upload] Upload completed but no uploadId found in URL');
      console.error('[Mux Upload] Upload URL:', uploadUrl);
  ...
  useEffect(() => {
    const fetchUploadUrl = async () => {
      try {
        setIsLoading(true);
        console.log('[Mux Upload] Fetching upload URL...');
        ...
        console.log('[Mux Upload] Upload URL received:', url);
        setUploadUrl(url);
        ...
        if (urlParts.length > 1) {
          ...
          console.log('[Mux Upload] Upload ID extracted early:', extractedUploadId);
        ...
        console.error('[Mux Upload] Failed to fetch upload URL:', error);
  ...
  return (
    <div className="w-full">
      <div className="space-y-4">
        ...
        {/* Processing State - Show when upload completes but video is still processing */}
        {isProcessing && (
          <div className="flex items-center justify-center p-4 bg-blue-50 rounded-lg">
            ...
            <div className="text-xs text-gray-500 ml-4">
              ⏱️ Long videos may take up to 20 minutes
            </div>
          </div>
        )}
      </div>
