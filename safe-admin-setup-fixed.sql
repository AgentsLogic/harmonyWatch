...
-- Step 11: Create admin user profile for existing auth user (safer approach)
-- First check if the profile already exists, if not create it
DO $$ 
BEGIN
    -- Check if admin profile already exists
    IF NOT EXISTS (
        SELECT 1 FROM user_profiles 
        WHERE user_id = '65163d49-94dd-442d-960c-0e070aa39157'::uuid
    ) THEN
        -- Check if the auth user exists before creating profile
        IF EXISTS (
